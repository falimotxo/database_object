...
from database_object_module.data_model import DatabaseObject, DatabaseObjectResult, DatabaseObjectException, \
    ErrorMessages
from database_object_module.impl.access_database_factory import AccessDatabaseFactory, AccessDatabase
...
class DatabaseObjectModule(object):
    ...
    def get(self, schema: str, object_name: str, conditions: tuple = (('_id', '!=', '')),
        ...
        :param condition: tuple of tuple conditions
        :type schema: str
    ...
    def put(self, schema: str, object_name: str, data: dict) -> DatabaseObjectResult:
        ...
        # Check that _id can not be greather than 12 characters
        if len(data[AccessDatabase.ID_FIELD]) > 12:
            raise DatabaseObjectException(ErrorMessages.MAX_ID_ERROR)

        schema_collection = schema + '_' + object_name
        ret = self.access_db.put(schema_collection, data)
        return DatabaseObjectModule._get_data_object_result_from_json('put', ret)

    def update_object(self, schema: str, object_name: str, data: DatabaseObject, conditions: tuple = (('_id', '!=', '')),
                      criteria: str = '', native_criteria: bool = False) -> DatabaseObjectResult:
        """
    ...
    def update(self, schema: str, object_name: str, data: dict, conditions: tuple = (('_id,' '!=', '')),
               criteria: str = '', native_criteria: bool = False) -> DatabaseObjectResult:
        """
        ...
        # _id can not exist
        if AccessDatabase.ID_FIELD in data:
    ...
    def remove(self, schema: str, object_name: str, condition: tuple = ('_id', '!=', ''),
        ...
        :param schema: connection schema
        :param object_name: object type to save
        :param condition: tuple condition
        :param criteria: criteria to search
        :param native_criteria: use native criteria from database or not
        ...
        schema_collection = schema + '_' + object_name
        ret = self.access_db.remove(schema_collection, condition, criteria, native_criteria)
